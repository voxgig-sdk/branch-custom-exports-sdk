...
class BranchCustomExportsMakeRequest
{
    public static function call(BranchCustomExportsContext $ctx): array
    {
        ...
        if ($fetch_err) {
            ...
            $response = new BranchCustomExportsResponse(['err' => $ctx->make_error('request_no_response', 'response: undefined')]);
        } elseif (is_array($fetched)) {
            $response = new BranchCustomExportsResponse($fetched);
        } else {
