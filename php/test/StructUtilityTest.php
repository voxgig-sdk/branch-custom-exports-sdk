...
// BranchCustomExports SDK struct utility test
//
...
class StructUtilityTest extends TestCase
{
    ...
    {
        if (self::$run === null) {
            $runner = BranchCustomExportsOmni::makeStructRunner(
    ...
    {
        return BranchCustomExportsOmni::objmodel(BranchCustomExportsOmni::argval($val));
    }

    /** A single-case fixture value on the comparison side. */
    private static function outval($val)
    {
        return BranchCustomExportsOmni::unmark($val);
    }

    /** A result normalised to the comparison model. */
    private static function resval($val)
    {
        return BranchCustomExportsOmni::structfix($val, Struct::undef());
    }
        ...
        $this->runsection(
            ...
            function ($input) {
                ...
                if (is_object($store) && $haspath && Struct::undef() !== $val) {
                    ...
                    if (json_encode(BranchCustomExportsOmni::structfix($now, Struct::undef()))
                        !== json_encode(BranchCustomExportsOmni::structfix($val, Struct::undef()))) {
