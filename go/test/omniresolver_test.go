...
import (
	...
	omni "github.com/voxgig-sdk/branch-custom-exports-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.BranchCustomExportsError); is {
				out := map[string]any{
					"name":    "BranchCustomExportsError",
...
func omniCtx(arg any, client *sdk.BranchCustomExportsSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.BranchCustomExportsSDK); is {
			client = live
	...
	if resMap, ok := ctxmap["result"].(map[string]any); ok {
		...
		if errMap, ok := resMap["err"].(map[string]any); ok {
			if msg, ok := errMap["message"].(string); ok {
				ctx.Result.Err = &sdk.BranchCustomExportsError{Msg: msg}
			}
		}
	}
...
func omniErrArg(arg any) error {
	...
	return &sdk.BranchCustomExportsError{Msg: msg, Code: code}
}
