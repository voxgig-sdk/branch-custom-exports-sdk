...
describe('InlineResponse200GetExportDirect', async () => {
  ...
  test('direct-exists', async () => {
    const sdk = new BranchCustomExportsSDK({
  ...
  test('direct-load-inline_response_200_get_export', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-inline_response_200_get_export', setup.live)) return
    if (skipIfMissingIds(t, setup, ["request_handle01"])) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['BRANCH_CUSTOM_EXPORTS_TEST_INLINE_RESPONSE_200_GET_EXPORT_ENTID']
