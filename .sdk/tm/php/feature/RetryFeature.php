...
// BranchCustomExports SDK retry feature
...
class BranchCustomExportsRetryFeature extends BranchCustomExportsBaseFeature
{
    ...
    public function init(BranchCustomExportsContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (BranchCustomExportsContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(BranchCustomExportsContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(BranchCustomExportsContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
