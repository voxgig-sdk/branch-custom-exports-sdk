# BranchCustomExports SDK test runner
...
class BranchCustomExportsTestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            BranchCustomExportsTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = BranchCustomExportsTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = BranchCustomExportsTestRunner.getenv("BRANCH_CUSTOM_EXPORTS_TEST_LIVE")
        override = BranchCustomExportsTestRunner.getenv("BRANCH_CUSTOM_EXPORTS_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = BranchCustomExportsTestRunner.getenv(key)
        ...
        explain = BranchCustomExportsTestRunner.getenv("BRANCH_CUSTOM_EXPORTS_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["BRANCH_CUSTOM_EXPORTS_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if BranchCustomExportsTestRunner._test_control is not None:
            return BranchCustomExportsTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                BranchCustomExportsTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            BranchCustomExportsTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = BranchCustomExportsTestRunner.load_test_control()
...
def load_env_local():
    BranchCustomExportsTestRunner.load_env_local()


def env_override(m):
    return BranchCustomExportsTestRunner.env_override(m)


def entity_data(v):
    return BranchCustomExportsTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return BranchCustomExportsTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return BranchCustomExportsTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return BranchCustomExportsTestRunner.load_test_control()


def live_client_options():
    return BranchCustomExportsTestRunner.live_client_options()


def live_delay_ms():
    return BranchCustomExportsTestRunner.live_delay_ms()
