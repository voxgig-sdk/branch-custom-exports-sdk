...
//    only - a typed BranchCustomExportsContext there would make every ctx
...
final class BranchCustomExportsOmni
{
    ...
    public static function ctx_from_map($ctxmap, $client, $utility)
    {
        ...
        $ctx = new BranchCustomExportsContext($clean, null);
        ...
        if (isset($clean['spec']) && is_array($clean['spec'])) {
            $ctx->spec = new BranchCustomExportsSpec($clean['spec']);
        }

        if (isset($clean['result']) && is_array($clean['result'])) {
            $ctx->result = new BranchCustomExportsResult($clean['result']);
            if (isset($clean['result']['err']) && is_array($clean['result']['err'])) {
                $msg = $clean['result']['err']['message'] ?? '';
                $ctx->result->err = new BranchCustomExportsError('', is_string($msg) ? $msg : '');
            }
        }

        if (isset($clean['response']) && is_array($clean['response'])) {
            $ctx->response = new BranchCustomExportsResponse($clean['response']);
    ...
    public static function err_from_map($m): ?BranchCustomExportsError
    {
        if (!is_array($m)) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new BranchCustomExportsError(is_string($code) ? $code : '', $msg);
    }
