...
// BranchCustomExports SDK feature test
//
...
class FtCtrl extends BranchCustomExportsControl
{
...
class FtClient
{
    ...
    private BranchCustomExportsUtility $utility;

    public function __construct(BranchCustomExportsUtility $utility, array $options)
    {
    ...
    public function get_utility(): BranchCustomExportsUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (BranchCustomExportsContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public BranchCustomExportsContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (BranchCustomExportsContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = BranchCustomExportsConfig::make_feature($name);
    ...
    private static function buildUrl(BranchCustomExportsSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof BranchCustomExportsError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new BranchCustomExportsResponse($fetched);
        }
    ...
    private function fail(BranchCustomExportsContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (BranchCustomExportsContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
