# branch-custom-exports-mcp

[MCP](https://modelcontextprotocol.io) server exposing the BranchCustomExports SDK as
two agent tools — `branch-custom-exports_list` and `branch-custom-exports_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/branch-custom-exports-mcp)
make build

# 2. Provide credentials via the environment
export BRANCH_CUSTOM_EXPORTS_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user branch-custom-exports \
  -- /absolute/path/to/branch-custom-exports-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./branch-custom-exports-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// branch-custom-exports_list: first page of records
{ "entity": "inline_response_200_get_export" }
{ "entity": "inline_response_200_get_export", "query": { } }

// branch-custom-exports_load: one record by id
{ "entity": "inline_response_200_get_export", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/branch-custom-exports-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user branch-custom-exports \
     -- "$PWD"/dist/*/branch-custom-exports-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `branch-custom-exports_list` and `branch-custom-exports_load` tools now appear
   in new sessions. Ask the agent to *"list inline_response_200_get_export using branch-custom-exports"*
   and it calls `branch-custom-exports_list` with `{"entity":"inline_response_200_get_export"}`.
...
```sh
export BRANCH_CUSTOM_EXPORTS_APIKEY=sk_live_xxx            # API key
export BRANCH_CUSTOM_EXPORTS_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `branch-custom-exports_list` tool
...
### Call the `branch-custom-exports_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `branch-custom-exports_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `branch-custom-exports_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `BRANCH_CUSTOM_EXPORTS_APIKEY` | API key sent with every request. |
| `BRANCH_CUSTOM_EXPORTS_BASE` | Optional override of the API base URL. |
...
```sh
./branch-custom-exports-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"branch-custom-exports_load","arguments":{"entity":"inline_response_200_get_export","query":{"id":1}}}}'
```
