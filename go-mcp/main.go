// Command BranchCustomExports-mcp is an MCP server exposing the BranchCustomExports
// SDK as MCP tools.
//
// Two tools are registered:
//
//	BranchCustomExports_list  — list records of an entity
//	BranchCustomExports_load  — load a single record of an entity
//
...
func main() {
	...
	// Configure from the environment: BRANCH_CUSTOM_EXPORTS_APIKEY carries the API key and
	// BRANCH_CUSTOM_EXPORTS_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("BRANCH_CUSTOM_EXPORTS_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("BRANCH_CUSTOM_EXPORTS_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewBranchCustomExportsSDK(opts)
