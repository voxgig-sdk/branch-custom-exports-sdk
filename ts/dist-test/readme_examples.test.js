...
const __1 = require("..");
const SDK_NAME = 'BranchCustomExportsSDK';
...
const TEST_SEED = { "entity": { "inline_response_200_get_export": { "test01": { "id": "test01" } }, "inline_response_200_post_data_readiness": { "test01": { "id": "test01" } }, "inline_response_200_post_export": { "test01": { "id": "test01" } } } };
...
function isRunnable(code) {
    return (/new\s+BranchCustomExportsSDK\b/.test(code) ||
        /\bBranchCustomExportsSDK\.test\b/.test(code) ||
        /\bclient\s*\./.test(code));
}
...
function rewriteForRun(code) {
    ...
    out = out.replace(/new\s+BranchCustomExportsSDK\s*\([^)]*\)/g, () => SEEDED_CTOR);
    out = out.replace(/BranchCustomExportsSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR);
