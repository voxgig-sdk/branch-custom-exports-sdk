...
class BranchCustomExportsMakeError
{
    public static function call(?BranchCustomExportsContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new BranchCustomExportsContext([], null);
        }
        $op = $ctx->op ?? new BranchCustomExportsOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new BranchCustomExportsResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof BranchCustomExportsError) ? $err->msg : (string)$err;
        ...
        $sdk_err = new BranchCustomExportsError('', $msg, $ctx);
        ...
        if ($err instanceof BranchCustomExportsError) {
