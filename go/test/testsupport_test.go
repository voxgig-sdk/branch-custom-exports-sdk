...
func envOverride(m map[string]any) map[string]any {
	if os.Getenv("BRANCH_CUSTOM_EXPORTS_TEST_LIVE") == "TRUE" ||
		os.Getenv("BRANCH_CUSTOM_EXPORTS_TEST_OVERRIDE") == "TRUE" {
		for key := range m {
	...
	if explain := os.Getenv("BRANCH_CUSTOM_EXPORTS_TEST_EXPLAIN"); explain != "" {
