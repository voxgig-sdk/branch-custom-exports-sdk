# branch-custom-exports-cli

boru-driven command-line client **and** interactive REPL for the BranchCustomExports
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/branch-custom-exports-cli)
make build

# 2. See usage (words, entities, env vars)
./branch-custom-exports-cli --help

# 3. Provide credentials once, via the environment
export BRANCH_CUSTOM_EXPORTS_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./branch-custom-exports-cli load 1 inline_response_200_get_export            # {id:1} shorthand
./branch-custom-exports-cli load '{id:1}' inline_response_200_get_export       # explicit match map

# 5. Override the API base URL for a single call
BRANCH_CUSTOM_EXPORTS_BASE=https://api.example.com ./branch-custom-exports-cli load 1 inline_response_200_get_export

# 6. No arguments -> interactive REPL
./branch-custom-exports-cli
branch-custom-exports> load 1 inline_response_200_get_export
branch-custom-exports> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/branch-custom-exports-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/branch-custom-exports-cli load 1 inline_response_200_get_export
   ```
...
```sh
./branch-custom-exports-cli load 1 inline_response_200_get_export          # scalar shorthand for {id:1}
./branch-custom-exports-cli load '{id:1}' inline_response_200_get_export     # explicit match map
```
...
./branch-custom-exports-cli load 1 inline_response_200_get_export
```
...
Run with no arguments to open a REPL (prompt `branch-custom-exports>`). Each line is
evaluated as its own boru expression:

```text
$ ./branch-custom-exports-cli
branch-custom-exports> load 1 inline_response_200_get_export
branch-custom-exports> /help
branch-custom-exports> /quit
```
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `inline_response_200_get_export`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/branch-custom-exports-cli`. |
...
(`./branch-custom-exports-cli <expr>`) and interactively (the REPL), and expressions compose the
