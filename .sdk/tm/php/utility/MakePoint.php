...
// BranchCustomExports SDK utility: make_point

require_once __DIR__ . '/../core/Helpers.php';

class BranchCustomExportsMakePoint
{
    public static function call(BranchCustomExportsContext $ctx): array
    {
        if (isset($ctx->out['point'])) {
            ...
            if ($ctx->out['point'] instanceof BranchCustomExportsError) {
        ...
        if (count($op->points) === 1) {
            ...
            foreach ($op->points as $p) {
                $select_def = BranchCustomExportsHelpers::to_map(\Voxgig\Struct\Struct::getprop($p, 'select'));
                $found = true;
            ...
            if ($reqselector) {
                $req_action = \Voxgig\Struct\Struct::getprop($reqselector, '$action');
                if ($req_action && $point) {
                    $point_select = BranchCustomExportsHelpers::to_map(\Voxgig\Struct\Struct::getprop($point, 'select'));
